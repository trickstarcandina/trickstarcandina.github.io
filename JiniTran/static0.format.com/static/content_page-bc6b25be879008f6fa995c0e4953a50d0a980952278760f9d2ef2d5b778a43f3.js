...
;(function($) {
 ...
 _4ORMAT.$.validator.addMethod("alphanumeric", function(value, element) {                                                                                                                                                                                                     
...
!function ($) {
  ...
  * =============================== */
  ...
  , hide: function () {
      ...
      function removeWithAnimation() {
        var timeout = setTimeout(function () {
          $tip.unbind($.support.transition.end).remove()
        }, 500)
  ...
  , destroy: function () {
      this.hide().$element.unbind('.' + this.type).removeData(this.type)
    }
...
}(typeof(_4ORMAT) === 'object' ? _4ORMAT.jQuery : window.jQuery);
(function(){
  var $ = this.jQuery,
      that =  null;

  /**
   * Create and show a new modal message.
   * @constructor
   * @param {(string|Object)} options - Either a string that will be the modal message text or an options object.
   * @param {Object} options.target_el - jQuery collection of the element in which the modal message will be placed.
   * For example:
   *   $('body')
   * @param {string} options.title - The text shown as the modal message's title.
   * For example:
   *   "My Title"
   */
...
// Public facing javascript for content pages
// It's included in the content iframe when the page is flagged to be
// rendered in 'edit mode'
(function() {
  var $ = this.$;
  var ModalMessage = _4ORMAT.ModalMessage;

  var ContactForm = {};
  this.ContactForm = ContactForm;

  var formSubmitTextValues = {};

  function beginFormInit() {
    if (typeof _4ORMAT.contact_forms === "object") {
      $(".email_form[data-editable-type='contact-form']").each(function(_i, form) {
        var $form = $(form);

        if ($form.data("contactFormType") !== "mailing-list") {
          ContactForm.initForm($form);
        }
      });
    }
  }

  ContactForm.initForm = function(form) {
    if (_4ORMAT_DATA && !_4ORMAT_DATA.page.content_editable && !_4ORMAT_DATA.preview) {
      $.ajax("/public_heartbeat").fail(function() {
        form.find("input, textarea").prop("disabled", true);
        form.find("input[type=submit]").attr("originalValue", form.find("input[type=submit]").val());
        form.find("input[type=submit]").val("Form is unavailable, please try again later");
      });
    }

    ContactForm.addValidations(form);

    // fixme - $(window).resize doesnt work but window.onresize does
    window.onresize = function() {
      $("div.tooltip").remove();
    };
  };

  // validation rules by data-validate type
  ContactForm.validationRules = {
    email: { email: true, required: true },
    required: { required: true },
  };

  // messages to display to the user when validating
  ...
  ContactForm.addValidations = function(form) {
    var rules = {};
    var messages = {};
    var name;
    var type;

    // A simple system that is specifically not the HTML5 validation spec so
    // that we don't have two validations going on.
    //
    // <input> - no validation
    // <input data-validate='required'> - validated presence
    // <input data-validate='email'> - validates presence and emali format
    form.find("*[data-validate]").each(function(i, node) {
      node = $(node);
      type = node.data("validate");
      name = node.attr("name");

      rules[name] = ContactForm.validationRules[type];
      messages[name] = ContactForm.validationMessages[type];
    });

    // Validations currently disabled in editor mode. Rendering the tooltips is
    // broken because of the page position of the iframe.
    if (!_4ORMAT_DATA.page.content_editable) {
      form.validate({
        ...
        errorElement: "p",
        errorClass: "invalid",
        errorPlacement: ContactForm.errorPlacement,
        unhighlight: ContactForm.unhighlight,
        submitHandler: ContactForm.submitHandler,
      });
    }
  };

  // callback that places validation error on the page
  ContactForm.errorPlacement = function(error, element) {
    $(element)
      .tooltip("destroy")
  ...
  // callback that unhighlights a previously highlighted field
  ContactForm.unhighlight = function(element) {
    $(element).tooltip("destroy");
  };
  ContactForm.submitHandler = function(form) {
    if (_4ORMAT_DATA.page.content_editable) {
      ContactForm.previewSubmit();
    } else {
      ContactForm.saveSubmitValue(form);
      ContactForm.disableSubmit(form);
      $.ajax({
        ...
        .done(ContactForm.sendSuccess)
        .done(function() {
          $(this)
            .find("textarea")
            .val("");
        })
        .fail(ContactForm.sendError);
    }
    return false; // required to prevent the form from submitting?
  };

  ContactForm.disableSubmit = function(form) {
    $(form)
  ...
  ContactForm.saveSubmitValue = function(form) {
    var formSubmitValue = $(form)
      .find("input[type=submit]")
      .val();
    formSubmitTextValues[$(form).attr("id")] = formSubmitValue;
  };

  ContactForm.previewSubmit = function() {
    new ModalMessage({
      title: "Note",
      text: "When this button is clicked by visitors to your site, it will send you this form in an email.",
    });
  };

  ContactForm.sendSuccess = function() {
    ...
    $(this)
      .find("input[type=submit]")
      .attr("disabled", "disabled")
      .addClass("f_form_send_success")
      .val("Message Sent");

    var that = this;
    var resetFormTimeout = 2000;
    setTimeout(function() {
      $(that)
        .find("input[type=submit]")
        .removeAttr("disabled")
        .val(formSubmitTextValues[formId]);
    }, resetFormTimeout);

    $(this)
      .find("fieldset")
      .removeClass("_4ORMAT_module_input_active");
  };

  ContactForm.sendError = function() {
    var formId = $(this).attr("id");
    $(this)
      .find("input[type=submit]")
      .removeAttr("disabled")
    ...
    new ModalMessage({
      title: "Warning",
      text: "We could not deliver your message. Please try again later.",
